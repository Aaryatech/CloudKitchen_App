...
class BulkOrderRepo{
  ...
  Future<HttpResponse> placeBulkOrder(BulkOrderModel order) async{
    ...
    String params ='?name=${order.name}&number=${order.number}&email=${order.email}&time=${order.time}&people=${order.people}&address=${order.address}';

    await httpClient.post('${endPoints.Auth().bulkOrder}$params').then((responce){
    ...
    await httpClient.post(endPoints.Auth().settingsForApp).then((responce){
      ...
      if(responce.statusCode==200){
        ...
        httpResponse.message='Successful';
         ...
         httpResponse.data=DairyMartModel.fromJson(responce.data['newSetting']);
