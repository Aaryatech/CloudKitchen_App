...
class GrievanceListRepo {
  ...
  Future<HttpResponse> getGreivance(String custId) async {
    ...
    await httpClient
        .post('${endPoints.Auth().grievanceList}$params')
        .then((responce) {
      print(responce);
  ...
  Future<HttpResponse> getGreivanceTypes() async {
    ...
    await httpClient
        .post('${endPoints.Auth().getGrievancetype}')
        .then((responce) {
      if (responce.statusCode == 200) {
        httpResponse.status = responce.statusCode;
        httpResponse.message = 'Successful';
  ...
  Future<HttpResponse> addGreivance(
      int orderId, int typeId, String dateTime, int userId, int id) async {
    HttpResponse httpResponse = HttpResponse();
